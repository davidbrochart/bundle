...
--        bit 1  - Channel 1 (ap_ready)
--        others - reserved
-- 0x10 : Data signal of mem_i_V
--        bit 1~0 - mem_i_V[1:0] (Read/Write)
--        others  - reserved
-- 0x14 : reserved
...
architecture behave of ddr2fpga_ctrl_s_axi is
    ...
    constant ADDR_MEM_I_V_DATA_0   : INTEGER := 16#10#;
    constant ADDR_MEM_I_V_CTRL     : INTEGER := 16#14#;
    constant ADDR_DATA_NB_V_DATA_0 : INTEGER := 16#18#;
    constant ADDR_DATA_NB_V_CTRL   : INTEGER := 16#1c#;
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_MEM_I_V_DATA_0 =>
                        rdata_data <= RESIZE(int_mem_i_V(1 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    mem_i_V              <= STD_LOGIC_VECTOR(int_mem_i_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_MEM_I_V_DATA_0) then
                    int_mem_i_V(1 downto 0) <= (UNSIGNED(WDATA(1 downto 0)) and wmask(1 downto 0)) or ((not wmask(1 downto 0)) and int_mem_i_V(1 downto 0));
                end if;
            end if;
        end if;
    end process;
